...
public:
    ...
    void operator()(function::function<real>& f, line_search::base_line_search<real>& ls, method_params<real>& params) {
        ...
        while(la::norm(gragient_curr) > params.epsilon && this->iter_count < params.max_iterations && fabs(fprev - fcur) / (1 + fabs(fcur)) > params.working_precision){
